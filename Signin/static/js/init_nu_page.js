	...
	error: function (response) {
  	console.log(response);
  		...
  		swal({
            ...
            confirmButtonText: "Грустно",
            closeOnConfirm: false
        });},
			success: function (response) {
  	console.log(response);
  	var error_login = "";
  	for (var q in JSON.parse(response).error_login){
  		error_login += JSON.parse(response).error_login[q] + "\n";
	};
  	swal({
			title: "Perfecto! Bellissimo! ",
             type: "success",
			text: "Всего было оптравлено: "+ JSON.parse(response).count + ". \n Создано: " + JSON.parse(response).success + ".\n Не создано: " + JSON.parse(response).errors + ", из них, это: \n" + error_login,
