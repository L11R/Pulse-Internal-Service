(function() {
  ...
  "async": true,
  "crossDomain": true,
  "url": '/get_leroy_parcels/',
  "method": "POST",
  "processData": false,
  "contentType": false,
  "mimeType": "multipart/form-data",
  "data": JSON.stringify({"cookie": $.cookie('token') }) };
    $.ajax(settings)
	.done(function (response) {
  //console.log(response);
  var arr = [];
  //var qq = JSON.parse(JSON.parse(response));
  var hh = JSON.parse(response).content.results;
  console.log(JSON.parse(response).content.results[1]);
      //[0].fields
  for (var i=0; i<hh.length; i++){
      arr.push(JSON.parse(JSON.stringify(hh[i])))
  }
  db.clients = arr;
...
MyDateField.prototype = new jsGrid.Field({
    ...
    itemTemplate: function(value) {
        if (value) { var now_doc = moment(value);
        now_doc.lang('ru');
        return now_doc.format('DD MMMM YYYY') } else { return ''
       }
...
jsGrid.fields.date = MyDateField;
  $("#jsgrid_1").jsGrid({
        ...
        fields: [
            ...
            { name: "point_number", title: "Терминал", type: "number", width: 150 },

            { name: "delivery_address", title: "Адрес", type: "text", width: 150 },


            { name: "receiver.phone", title: "Телефон", type: "text", width: 150 },
            { name: "date_added", title: "Дата создания", type: "date", width: 150},
            { name: "date_delivered", title: "Дата доставки", type: "date", width: 150 },
            { name: "date_received", title: "Дата получения", type: "date", width: 150 },
